...
def _cross_entropy_forward(
    ...
    """#这里面公式推导,来加速ce的计算.
        Cross Entropy Loss = 1/n sum [ -yi log(Pi) ] # 理解交叉熵这个公式.  yi是真实的i事件发生的概率,  pi是我们算出来的概率, 那么他们之间差距多大呢.首先我们 知道 最后算概率都是做指数平滑再归一化(就是下一行43行注释), 所以最后的概率要先做log把他归一化. 对于分类问题 就是一个yi=1, 其他yi=0,而log(pi)当pi=1时候,他等于0,这时候loss最小=0. 当pi<1时候他loss>0.所以对于分类问题特别适合交叉熵来当loss.
        Pi = exp(xi) / sum(exp(xi)) # xi是网络输出.
        CE_i = -y log(p) = -y log[ exp(x) / sum(exp(x)) ] # 这行计算的是ce的每一项.
             = -y [ x - log[sum(exp(x))] ] # log化简
             = y * (log[sum(exp(x))] - x)
        If y == 0: CE_i = 0
        If y == 1: CE_i = logsumexp - x

        logsumexp is also stable #stable就是x波动小, y波动也小.不会剧烈的波动导致loss不容易收敛.
        Take    y =         log[sum(exp(x))]
           exp(y) =             sum(exp(x))
           exp(y) =             sum(exp(x - c)*exp(c)) Since e^(x-c)*e^c = e^x
           exp(y) =      exp(c)*sum(exp(x - c))
               y  = log(exp(c)*sum(exp(x - c)))
               y  = c + log[sum(exp(x - c))]       #我们利用这个公式来化简, 因为这个公式x-c可以让这个数值变小. 来防止exp指数爆炸. 因为这里面x是网络输出, 是实数范围.所以再算exp会经常爆炸, 但是减去最大值, 就让他范围从0到1了.可以精确计算了!!!!!下面代码就是这些的代码实现.
...
def _chunked_cross_entropy_forward( # chunk加速计算ce
    logits_ptr        ,
    logits_row_stride ,
    loss_ptr          ,
    logsumexp_ptr     ,
...
def _cross_entropy_backward(
    ...
    """
        ...
        d/dx logsumexp = exp(x) / sum(exp(x)) = softmax(x) #就是简单导数计算. 这里面sum对索引i求和.
