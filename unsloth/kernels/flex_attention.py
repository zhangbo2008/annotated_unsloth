...
# 一种mask 是sliding_window_masker  一种是causual
import torch
...
# Flex Attention supported from torch 2.5 onwards only 更加自由的attention计算.可以定制化自己的参数.
try:
...
# 遵循Gemini 1.5（Gemini Team, 2024），我们在每个注意力层和最后一层中对logit进行软上限限制，使得logit的值保持在-soft_cap和+soft_cap之间。更具体地说，我们设置logit为： [-t,t]
    # Logit softcapping # 实现attn_logit_softcapping.
    ...
    def slow_attention_softcapping(Q, K, V, causal_mask, self, bsz, q_len):
        ...
        A = t * torch.tanh(A / t) # Logit softcapping # 逻辑概率进行软上限. A是注意力, 让注意力不超过 t 和-t
    ...
    def causal_masker(b, h, q_idx, kv_idx):
        return q_idx >= kv_idx # q索引大于kv时候都要做遮挡, 不计算注意力.
    pass
    ...
    def sliding_window_masker(size = 4096):
        def sliding_window(b, h, q_idx, kv_idx): 
            ...
            window_mask = q_idx - kv_idx <= size  # 等于因果mask,再交上. windows mask, windows_mask是让q 和kv 索引足够近才行. 太远了就不算注意力了.这里默认4096.这么设置会加速计算.
