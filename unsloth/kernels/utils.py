...
# gemv: general matrix  vector 的乘法.
if HAS_CUDA_STREAM:
...
def matmul_lora(X, W, W_quant, A, B, s, out = None):
    ...
    if X.dim() == 3:
        ...
        X = X.view(-1, X.shape[-1]) # 因为矩阵乘法 A*B只需要A的列数跟B的行数一样就能乘.所以我们X的最后一个维度保持, 其他都展平也是可以乘的.
        reshape = True
    else:
        reshape = False
    pass
    ...
    if A is not None: # A,B是lora的权重
        # LoRA is enabled
        A, B = A.t(), B.t()
        out += (X @ A.to(dtype)) @ (s * B.to(dtype)) # 原始矩阵再加上lora的结果即可.
    pass
    
    return out.view(batch, seq_len, -1) if reshape else out # 最后把452行展平的叠回去.
pass
